...
const Product = async ({ params }: { params: { product: string } }) => {
  ...
  const productData = products.find((item) => item.id === Number(product));

  return (
    <div className='m-[4rem]'>
      <h1 className='text-3xl mb-[1rem]'>{productData?.name}</h1>
      ...
      <p className='text-2xl mt-[1rem]'>Price: ${productData?.price}</p>
    </div>
  );
};
